...
## Reglas de oro
1. La IA NO inventa datos: usa `[CONFIRMAR: ...]` cuando falte un dato. Esos casos se registran
   como "huecos" en `utils/kb_store.py` para que los admins los llenen.
2. Modelo: `claude-opus-4-8` (Anthropic SDK). No cambiar sin pedir.
3. Tono White-Glove (no vendedor) + método LAER + doble audiencia (padre/estudiante). Todo eso
   vive en `core/assistant.py` (`_REGLAS`).
